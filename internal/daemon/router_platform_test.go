...
func TestHelpTextUsesDiscordSlashCommands(t *testing.T) {
	...
	text := platformHelpText("discord", "")
	for _, want := range []string{"/start", "/help", "/snapshot [lines] [image|text]", `prefix text commands with "tmux:"`} {
		if !strings.Contains(text, want) {
			t.Fatalf("platformHelpText(discord) missing %q in %q", want, text)
		}
	}
	for _, unwanted := range []string{"\ntmux: start", "\ntmux: help"} {
		if strings.Contains(text, unwanted) {
			t.Fatalf("platformHelpText(discord) unexpectedly contains %q in %q", unwanted, text)
		}
	}
}
...
func TestHelpTextUsesWeixinSnapshotTextGuidance(t *testing.T) {
	...
	text := platformHelpText("weixin", "")
	for _, want := range []string{`/snapshot [lines] [text]`, `forces "/snapshot" to text output`, `placeholder boxes`} {
		if !strings.Contains(text, want) {
			t.Fatalf("platformHelpText(weixin) missing %q in %q", want, text)
		}
	}
	if strings.Contains(text, "/snapshot [lines] [image|text]") {
		t.Fatalf("platformHelpText(weixin) unexpectedly contains image snapshot usage in %q", text)
	}
}
...
func TestRouterDiscordPromptMentionsPrefixedReplyInChannels(t *testing.T) {
	...
	service := newFakePaneService()
	messenger := &fakeMessenger{platform: "discord", commandPrefix: "bot:"}
	replyBus := NewReplyBus(messenger, store, termrender.Options{})
	router := NewRouter(service, NewPaneRegistry(service), store, replyBus, NewFollowManager(service, replyBus, 20), 120, nil, "")
	...
	messages := messenger.snapshot()
	last := messages[len(messages)-1]
	if !strings.Contains(last.Text, `reply with "bot: <value>"`) {
		t.Fatalf("prompt text = %q, want custom Discord channel reply hint", last.Text)
	}
}

func TestRouterDiscordHelpUsesCustomCommandPrefix(t *testing.T) {
	t.Parallel()
	...
	if err := router.HandleMessage(ctx, discordMessage("C123", "1", "/help")); err != nil {
		t.Fatalf("HandleMessage(help) error = %v", err)
	}

	last := messenger.snapshot()[0]
	if !strings.Contains(last.Text, `prefix text commands with "bot:"`) {
		t.Fatalf("help text = %q, want custom discord prefix guidance", last.Text)
	}
	if !strings.Contains(last.Text, "bot: current") {
		t.Fatalf("help text = %q, want custom discord prefix example", last.Text)
	}
}
